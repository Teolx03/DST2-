...
<jsp:include page="nav.jsp">
  <jsp:param name="active" value="query_result" />
</jsp:include>
        ...
        <ul class="list-group">
          <c:forEach var="item" items="${results}">
            <li class="list-group-item">
              <strong>Drug ID:</strong> ${item.drugId}<br>
              <strong>Drug Name:</strong> ${item.drugName}<br>
              <strong>Recommendation:</strong> ${item.recommendation}<br>
              <strong>Summary:</strong> ${item.summaryMarkdown}
            </li>
          </c:forEach>
        </ul>
      </c:if>
      <c:if test="${empty results}">
        <p class="text-muted">Incorrect drug ID, name, or nonexistent dosing guideline</p>
      </c:if>
    </main>
  </div>
</div>
